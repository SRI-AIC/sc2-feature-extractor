...
class ForceFactorsExtractor(FeatureExtractor):
    ...
    def features_descriptors(self) -> List[FeatureDescriptor]:
        desc = []
        for i, ff in enumerate(self.config.force_factors):
            levels = [level[NAME_PARAM_STR] for level in ff.levels] if self._categorical else [0., 1.]
            for g in self._friendly_filters[i]:
                desc.append(FeatureDescriptor(
                    f'{ff.name}_{FRIENDLY_STR}_{g}',
                    FeatureType.Categorical if self._categorical else FeatureType.Real, levels))
            for g in self._enemy_filters[i]:
                desc.append(FeatureDescriptor(
                    f'{ff.name}_{ENEMY_STR}_{g}',
                    FeatureType.Categorical if self._categorical else FeatureType.Real, levels))
        return desc
    ...
    def extract(self, ep: int, step: int, obs: NamedDict, pb_obs: ResponseObservation) -> \
        ...
        for i, ff in enumerate(self.config.force_factors):
            ...
            def _add_groups_features(_filter):
                ...
                for g in _filter:
                    feature = DEFAULT_FEATURE_VAL if self._categorical else np.nan
